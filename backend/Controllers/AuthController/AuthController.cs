    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.CvBase64))
            {
                ...
                {
                    return BadRequest($"Error processing CV file: {ex.Message}");
                }
            }
        ...
        public async Task<IActionResult> RegisterFirstAdmin([FromBody] RegisterModel model)
        {
            ...
            // Check if any RH user already exists
            if (await _userManager.Users.OfType<RH>().AnyAsync())
            {
                return BadRequest("An admin user already exists. Please use the register-admin endpoint.");
            }
            ...
            var admin = new RH
            {
                ...
                EmailConfirmed = true // Auto-confirm email for admin users
            };
            ...
            // Add the RH role
            await _userManager.AddToRoleAsync(admin, admin.UserType.ToString());

            // Add UserType claim
            await _userManager.AddClaimAsync(admin, new System.Security.Claims.Claim("UserType", admin.UserType.ToString()));

            // Generate JWT token
            var token = GenerateJwtToken(admin);

            return Ok(new { Message = "First admin user created successfully", Token = token, user = admin });
        }
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegisterModel model)
        {
            ...
            return Ok(new { Message = "Admin user created successfully", Token = token, user = admin });
        }
